...
def getHashtags(topic_decomp_file,id_str_file,output_file):
    ...
    for ii in range(len(id_str)):
        ...
        print(hashtags_)
        print(ii)
        for each_hashtag_ in hashtags_:
            fout.write(str(id_str[ii]) + ' ' + str(docs_sorted_topics[ii]) + ' ' + each_hashtag_ + '\n');
            print(str(id_str[ii]) + ' ' + str(docs_sorted_topics[ii]) + ' ' + each_hashtag_);
...
def hashtag_per_topic_count(no_topics,input_file): 
    ...
    for top in range(no_topics):
        ...
        dict_top = dict();
        fout = open(directory + \
                '/output/BTM_OUTPUT/models/'+run_number+'/topic_hashtag_counts_trial_2/topic_hashtag_count_'+str(top)+'.txt','w');
...
def get_relevant_hashtags(tweet):
    ...
    dominant_topic_temp = list(sorted(enumerate(doc_by_topic), key = lambda x: x[1], reverse=True));
    dominant_score_1 = dominant_topic[1];
    dominant_topic = dominant_topic[0]
    print(dominant_topic,dominant_topic_temp[1][0])
    hashtag_count_dict = eval(open('/Users/jananikalyanam/Documents/insight_application/PROJECT/output/BTM_OUTPUT/models/' + \
            'first_run/topic_hashtag_counts_trial_2/topic_hashtag_count_' + str(dominant_topic) + '.txt').read())
    hashtag_count_dict = hashtag_count_dict.items();
    hashtag_count_dict = sorted(hashtag_count_dict, key = lambda x: x[1], reverse = True);
    hashtag_count_dict_1 = list(map(lambda x: x[0], hashtag_count_dict));

    return_list = [];

    no = 0;
    for each_ in hashtag_count_dict_1:
        if isEnglish(each_):
    ...
    for ii in range(len(return_list)):
        if(len(return_list[ii].split('-')) > 1):
            return_list[ii] = ', #'.join(return_list[ii].split('-'));
            return_list[ii] = '#'+return_list[ii];
        else:
            return_list[ii] = '#'+return_list[ii];
